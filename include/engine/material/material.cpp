...
// * POINTS MATERIAL ///////////////////////////////////////////
Points_Material::Points_Material() : Material() {}

void Points_Material::build(VkRenderPass renderPass, VkExtent2D swapchainExtent, VkBuffer globalProjectionBuffer) {
    std::cout << "gaming" << '\n';
    // * Crear buffer para el struct GlobalProjectionInfo
    descriptorBufferInfo = {};
        descriptorBufferInfo.buffer = globalProjectionBuffer;
...
void Points_Material::setup_descriptor_set(VkCommandBuffer cmdBffr) {
	vkCmdBindDescriptorSets(cmdBffr, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSets, 0, 0);
}

void Points_Material::recreate(VkRenderPass renderPass, VkExtent2D swapchainExtent) {
    resize_cleanup();
    build_material_pipeline("shaders/points.vert.spv", "shaders/points.frag.spv", renderPass, swapchainExtent);
}

void Points_Material::build_material_pipeline(char const* vertShaderPath, char const* fragShaderPath, VkRenderPass renderPass, VkExtent2D swapchainExtent) {
    auto vertShaderCode = readFile(vertShaderPath);
    auto fragShaderCode = readFile(fragShaderPath);
