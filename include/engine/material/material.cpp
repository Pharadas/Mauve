...
void Material::recreate(VkRenderPass renderPass, VkExtent2D swapchainExtent) {
  resize_cleanup();
  std::cout << "creating pipeline with files shaders/basic.vert.spv and shaders/basic.frag.spv" << std::endl;
  build_material_pipeline("shaders/basic.vert.spv", "shaders/basic.frag.spv", renderPass, swapchainExtent);
}
...
Point_Material::Point_Material() : Material() {}

Point_Material::Point_Material(std::vector<Texture> textures, VkSampler* inputSampler) : Material() {
    texturesVector = textures;
    sampler = inputSampler;
}

void Point_Material::build_material_pipeline(const char *vertShaderPath, const char *fragShaderPath, VkRenderPass renderPass, VkExtent2D swapchainExtent) {
  auto vertShaderCode = readFile(vertShaderPath);
  auto fragShaderCode = readFile(fragShaderPath);
...
void Point_Material::build(VkRenderPass renderPass, VkExtent2D swapchainExtent, VkBuffer globalProjectionBuffer) {
  // * Guardar globalProjectionBuffer en un struct para poder usarlo
  VkDescriptorBufferInfo bufferInfo {};
      bufferInfo.buffer = globalProjectionBuffer;
      bufferInfo.offset = 0;
  ...
  build_material_pipeline("shaders/points.vert.spv", "shaders/points.frag.spv", renderPass, swapchainExtent);
}

void Point_Material::recreate(VkRenderPass renderPass, VkExtent2D swapchainExtent) {
  resize_cleanup();
  std::cout << "creating pipeline with files shaders/basic.vert.spv and shaders/basic.frag.spv" << std::endl;
  build_material_pipeline("shaders/points.vert.spv", "shaders/points.frag.spv", renderPass, swapchainExtent);
}
