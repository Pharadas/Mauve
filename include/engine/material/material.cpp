...
Textured_Material::Textured_Material(Texture texture, VkSampler sampler, VkRenderPass renderPass, VkExtent2D swapchainExtent) : Material(renderPass, swapchainExtent) {
    VkDescriptorImageInfo imageInfo{};
    imageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    imageInfo.imageView = texture._textureImageView;
    imageInfo.sampler = sampler;

    // * Build the descriptor sets
    builder = DescriptorBuilder::begin(&descriptorLayoutCache, &descriptorAllocator);
    builder.bind_image(0, &imageInfo, typesAndFlags[0].first, typesAndFlags[0].second);
    builder.build(descriptorSets, descriptorSetLayout);
...
void Textured_Material::build_material_pipeline(char const* vertShaderPath, char const* fragShaderPath, VkRenderPass renderPass, VkExtent2D swapchainExtent) {
    auto vertShaderCode = readFile(vertShaderPath);
    auto fragShaderCode = readFile(fragShaderPath);
...
// void Textured_Material::cleanup() {
//     std::cout << "destruyendo pipeline layout\n";
//     vkDestroyPipelineLayout(_device, pipelineLayout, nullptr);
//     std::cout << "destruyendo pipeline\n";
//     vkDestroyPipeline(_device, pipeline, nullptr);
//     std::cout << "terminado";
